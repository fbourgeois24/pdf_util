...
class PDF(FPDF):
	...
	def create_table(self, table_data, title='', data_size = 10, title_size=12, align_data='L', align_header='L', cell_width='even', x_start='x_default', \
		...
		# add title
		if title != '':
			self.multi_cell(0, line_height, title, border=border, align='j', ln=3, max_line_height=self.font_size)
		...
		if  not isinstance(col_width, list):
			if x_start:
				self.set_x(x_start)
			for datum in header:
				self.multi_cell(col_width, line_height, str(datum), border=border, align=align_header, ln=3, max_line_height=self.font_size)
				x_right = self.get_x()
			self.ln(line_height) # move cursor back to the left margin
			y2 = self.get_y()
			...
			for row in data:
				...
				for datum in row:
					if datum in emphasize_data:
						...
						self.multi_cell(col_width, line_height, datum, border=border, align=align_data, ln=3, max_line_height=self.font_size)
						self.set_text_color(0,0,0)
						self.set_font(style=default_style)
					else:
						self.multi_cell(col_width, line_height, str(datum), border=border, align=align_data, ln=3, max_line_height=self.font_size) # ln = 3 - move cursor to right with same vertical offset # this uses an object named self
				self.ln(line_height) # move cursor back to the left margin
